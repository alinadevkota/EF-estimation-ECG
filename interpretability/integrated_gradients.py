...
def plot_ig2_tp_fp_fn_tn(svs, y_scores, targets, cmap=plt.cm.Blues):
    ...
    for i in tqdm(range(n)):
        if predicted_labels[i] == 0 and targets[i] == 0:
            results["TP"].append(svs[0, i])  # True Positive
        elif predicted_labels[i] == 0 and targets[i] == 1:
            results["FP"].append(svs[0, i])  # False Positive
        elif predicted_labels[i] == 1 and targets[i] == 0:
            results["FN"].append(svs[1, i])  # False Negative
        elif predicted_labels[i] == 1 and targets[i] == 1:
            results["TN"].append(svs[1, i])  # True Negative
    ...
    im = axs.imshow(ys, cmap=cmap, vmin=-3, vmax=3)
    axs.figure.colorbar(im, ax=axs, shrink=0.4)
    ...
    for i in range(ys.shape[0]):
        for j in range(ys.shape[1]):
            axs.text(
                ...
                ha="center",
                va="center",
                color="white" if abs(ys[i, j]) > thresh else "black",
            )

    np.set_printoptions(precision=2)
    fig.suptitle(
        "Population-level Interpretation Showing Contribution of Each Lead",
        fontsize="large",
        y=0.72,
        x=0.455,
    )
    # Customize the colorbar thickness
    fig.tight_layout()
    # plt.legend(loc='upper right')  # Add a legend for the categories
    plt.savefig("ig2_tp_fp_fn_tn.png", dpi=300, bbox_inches="tight")
    plt.clf()
...
def plot_ig2(svs, y_scores, cmap=plt.cm.Blues):
    ...
    ys = np.array(ys)
    fig, axs = plt.subplots()
    im = axs.imshow(ys, cmap=cmap, vmin=-2, vmax=2)
    axs.figure.colorbar(im, ax=axs, shrink=0.8)
    fmt = ".2f"
    xlabels = leads
    ylabels = [0, 1] + ["AVG"]
    axs.set_xticks(np.arange(len(xlabels)))
    ...
    np.set_printoptions(precision=2)
    fig.tight_layout()
    plt.savefig("ig2.png", dpi=300, bbox_inches="tight")
...
def main():
    ...
    plot_ig2(ig_values, y_scores, cmap=plt.cm.coolwarm)
    plot_ig2_tp_fp_fn_tn(ig_values, y_scores, targets, cmap=plt.cm.RdBu)
