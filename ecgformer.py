...
class LinearEmbedding(nn.Sequential):
    ...
    def forward(self, x):
        ...
        return torch.cat(
            [einops.repeat(self.cls_token, "n e -> b n e", b=x.shape[0]), embedded],
            dim=1,
        )
        ...
        self.encoder = nn.ModuleList(
            [
                TransformerEncoderLayer(
                    embed_size=embed_size, num_heads=num_heads, expansion=expansion
                )
...
if __name__ == "__main__":
    ...
    print(ECGformer(4, 128, 5, 3, 192, 8, 4)(torch.rand(2, 128, 3)).shape)
