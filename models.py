...
class Net(nn.Module):
    def __init__(self, in_channels, regression=False):
        super(Net, self).__init__()
        self.flatten = nn.Flatten()
        self.fc1_temp = nn.Linear(in_channels * 2048, 1000)
        # self.fc2_temp = nn.Linear(1000, 200)
        self.dropout1 = nn.Dropout(0.25)
        self.dropout2 = nn.Dropout(0.5)
        self.fc_hidden = nn.Linear(1000, 200)

        self.bn = nn.BatchNorm1d(1000)
        self.regression = regression
        if self.regression:
            self.fc_final = nn.Linear(200, 1)
        else:
            self.fc_final = nn.Linear(200, 2)
